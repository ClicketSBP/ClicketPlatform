...
/* Update zoneDataname - ADMIN ONLY */
router.post("/zone/data/update/name", authenticate, admin, (req, res) => {
    if (req.granted) {
        if (Object.keys(req.body).length !== 3 || bodyValidator(req.body.zone_id, req.body.street, req.body.city)) {
            res.json({
            ...
            ZoneData.getZoneDataById(req.body.id, (err, zoneData) => {
                if (err) {
                    ...
                    zoneData.name = req.body.name;
                    ZoneData.updateZoneData(zoneData, req.body, (err) => {
                        if (err) {
                            res.json({
                                info: "Error during updating zoneData name",
                                success: false,
                                error: err.errmsg
                            });
                        } else {
                            res.json({
                                info: "Zone data name updated successfully",
                                success: true
                            });
                        }
                    });
